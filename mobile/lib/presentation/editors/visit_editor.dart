...
import '../visit_record_picker.dart';
...
Future<void> editVisit(
  ...
  final hasEntries = c.records.entries(pid, limit: 1).isNotEmpty;
  final missingEntryIds = selected
      .where((id) => c.records.entry(pid, id) == null)
      .toSet();
      ...
      MaterialPageRoute<void>(
        builder: (_) => EditorPage(
          ...
          content: (update) => [
            ...
            if (selected.any(missingEntryIds.contains)) ...[
              Text(
                context.tr(
              ...
              OutlinedButton(
                onPressed: () => update(
                  () => selected.removeWhere(missingEntryIds.contains),
                ),
                child: Text(context.tr('삭제된 원본 연결 제외')),
              ),
            ],
            if (!hasEntries) Text(context.tr('먼저 일기에 기록을 남겨 주세요.')),
            OutlinedButton.icon(
              icon: const Icon(Icons.checklist),
              label: Text(context.tr('기록 선택 · {0}개 선택됨', [selected.length])),
              onPressed: () async {
                final result = await Navigator.of(context).push<Set<String>>(
                  MaterialPageRoute(
                    builder: (_) => VisitRecordPicker(
                      c: c,
                      patientId: pid,
                      selected: selected,
                    ),
