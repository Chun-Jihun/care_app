...
Flutter 기록 앱의 첫 구현은 `mobile/`에 있다. [구현 전 실패 시나리오](./docs/non_ai_foundation_safety_plan.md)와 [실행·검증 안내](./mobile/README.md)를 함께 확인한다. 다음 작업은 실제 기기 검증과 입력 흐름 개선이다.

1. Android 실제 기기에서 사진·파일 선택·기기 인증·알림·재부팅·잠금 동작을 검증한다.
2. Mac/Xcode에서 iOS 빌드와 Keychain·사진·알림·앱 전환 가림을 검증한다.
3. 복약 반복 일정의 상세 조건, 대용량 사진·백업과 사용자 입력 편의 기능을 확장한다.
4. 연결된 OCR·음성 전사의 실기기 정확도·메모리·응답 시간을 검증하고 입력 편의를 개선한다.
5. 승인 snapshot과 임상 검수 전에는 생성형 의료 Q&A와 임상 위험 규칙을 연결하지 않는다.
