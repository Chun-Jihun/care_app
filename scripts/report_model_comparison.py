...
def _markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 실제 구현된 에이전트 구성",
        "",
        "아래는 제품의 최종 의료 아키텍처가 아니라, 이번 동일 T1 비교에서 실제 실행한 경로다. 후보 LLM 하나를 A1과 A4에 재사용하고 A2·A3·A5는 결정적 코드로 고정해 모델에 따른 차이를 A1·A4에 한정했다.",
        "",
        "```mermaid",
        "flowchart LR",
        ...
        "```",
        "",
        "흐름에서 `결정적`은 같은 입력에 같은 규칙을 적용하는 일반 코드라는 뜻이다. 따라서 현재 T1의 실제 생성형 에이전트 역할은 A1과 A4 두 곳이며, A2·A3·A5는 이름은 역할 계약을 따르지만 LLM 에이전트가 아니다. 모델은 DB나 도구를 직접 실행하지 않는다.",
        "",
        "### 비교 모델을 배치한 위치",
        "",
        "```mermaid",
        ...
        "```",
        "",
        "MedGemma와 Nanbeige의 1건 결과는 각각 A4와 A1 JSON 인터페이스 연결 가능성만 본 것이다. 세 모델의 T1 8건 결과와 같은 범위의 E2E 성능으로 비교하면 안 된다.",
        "",
        "## 지표를 보기 전에 알아야 할 용어",
        "",
