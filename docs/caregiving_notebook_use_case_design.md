...
8. 가족 공유, 병원 전송, 원격 동기화 및 EMR 연동은 초기 MVP에서 분리한다.
...
| UC-15 | 앱 잠금·백업·내보내기·삭제 | MVP 필수 | FR-14 |
| UC-16 | 공개 지원서비스와 간병인 지원 확인 | MVP 권장 | FR-12, FR-13 |
...
| `application` | UC-01~UC-18 실행 순서와 트랜잭션 조정 | 의료 규칙을 화면 코드에 작성 |
...
2. **로컬 데이터 기반:** 암호화 저장, 환자 분리, 앱 잠금, 수정 이력, 백업·복원과 삭제를 구현한다.
...
7. **간병 도우미:** UC-08, UC-11~UC-13을 로컬 RAG와 제한 생성으로 연결한다.
