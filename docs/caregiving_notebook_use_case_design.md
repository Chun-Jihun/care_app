...
| 객체 | 보존해야 할 핵심 정보 |
|---|---|
| `PatientContext` | 로컬 환자 ID, 질환 범주와 치료 단계; 선택적 식별정보와 의료진 지시는 각각 별도 객체로 저장 |
| `ClinicianInstruction` | 의료진 지시 원문, 입력·검증 수준, 의료진 연락·첨부자료 출처, 유효기간, 활성·대체·철회 상태와 이력 |
| `CareEntry` | 기록 시각, 작성자 역할·별칭, 기록 유형, 관찰 사실, 수행한 돌봄, 이후 변화 |
| `MedicationPlan` | 확인된 약 식별자, 처방 내용, 예정 시각, 선택적 추출 출처와 이전 계획을 덮어쓰지 않는 대체 이력 |
| `MedicationIntake` | 실제 복용 시각, 복용 여부, 누락·거부 이유, 관찰된 이상 반응 |
| `CareTaskSchedule` | 복약·식사·활동·진료 예정 시각, 완료 상태와 명시적으로 연결된 간병기록·복약 일정·진료 준비 중 하나 |
...
| `VisitPreparation` | 기간, 변화·복약·증상·질문 항목, 항목별 여러 원본 기록과 사용한 원본 버전, 사용자 확인·재검토 상태 |
| `EvidenceDocument` | 자료명, 기관, 발행·개정일, 페이지·절, 검수된 근거 span, 링크, 앱 검수일과 버전 |
| `GroundedAnswer` | 사용자 질문 메시지와 실제 표시된 assistant 메시지, 참고한 기록, 순서·위치가 있는 핵심 주장, 연결된 근거, 보류·연락 상태 |
| `ConversationThread` | 환자별 대화 ID, 생성·최근 사용 시각, 보관 여부, 보유기간, 자동삭제 예정일과 삭제 상태 |
| `ConversationMessage` | `user`·`assistant` 역할, 입력되거나 실제 표시된 불변 원문과 생성 시각 |
| `DeletionJob` | 여러 로컬 저장소의 삭제 단계, 재시도·완료 상태와 앱 재시작 후 재개 정보 |
| `AuditEvent` | 생성·수정·확인·삭제·내보내기와 사용한 모델·문서·규칙 버전; 전체 삭제 후에는 환자 연결자를 남기지 않음 |

## 9. 구현 순서

요구사항에 따라 기능 구현 전에 관련 안전 실패 시나리오와 시험을 먼저 정의한다.

아래 제품 구현에 앞서 합성·비식별 평가환경에서 [`A1~A5 역할·도구 사용 규약`](./agent_role_and_tool_contracts.md), 단일 에이전트와 역할 분리 토폴로지, trace 기반 실패 원인 분석을 먼저 검증한다. 이 선행 연구는 [`로컬 간병 에이전트 구성 및 성능평가 계획`](./agent_architecture_and_evaluation_plan.md)을 따르며, 로컬 간병일기와 저장 기반의 MVP 우선순위 자체를 대체하지 않는다.
