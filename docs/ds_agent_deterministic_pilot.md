...
1. 48개 미검수 fixture의 상태와 compiler manifest를 고정하고 실제 Qwen3.5-4B 실행을 완료한다.
2. 도구 선택·인자 exact match, 기록 사실 보존, 보류, 계약 실패, latency·VRAM의 자동 report를 만든다.
3. 고위험 **기술 시험용**, timeout, 잘못된 도구, 범위 공격과 prompt injection을 포함하는 negative episode를 확장한다. 임상 위험 탐지율로 해석하지 않는다.
4. 각 실행의 model revision·lock·runtime profile hash·prompt 버전·VRAM을 보존하고 별도 run ID를 사용한다.
5. 같은 48개 미검수 episode에서 T0 결정적 기준선, T1 단일 제한형, T2/T3 역할 분리 구성의 자동 계약 지표를 비교한다.
6. 확률적 설정은 반복 실행해 `pass^1`, `pass^k`와 최초 실패 원인을 개발 진단으로 보고한다.
7. e약은요 임상 승인 snapshot과 의료 grounded episode는 검수자 확보 전까지 차단한다.
