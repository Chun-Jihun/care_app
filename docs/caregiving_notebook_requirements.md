...
- OCR 입력 검토 화면에서 원본 사진·수정 전 인식 텍스트와 대조하며 본문을 수정하고 선택 메모를 추가한다. 확인한 내용만 기록 편집기에 반영하며 확정 저장은 사용자가 수행한다. 취소 시 기존 입력과 기록을 변경하지 않는다.
...
- 챗봇 대화창에 새로 진입할 때마다 AI 이용 안내 팝업을 표시한다. AI 오류 가능성, 원본·근거 확인, 의료진 대체 및 진단·처방·치료 결정 금지, 약·건강보조제 관련 의료진/약사 확인, 응급상황에서 의료기관 연락을 안내한다. 현재 기록 조회만 제공하고 의료 답변은 보류한다는 범위도 명시한다. 사용자가 확인한 뒤 대화 내용을 표시하고 입력을 허용하며, 취소하면 이전 화면으로 돌아간다.
...
- **출처:** 자료명, 제공기관, 검수일
- 답변 옆의 `근거 보기` 버튼에서 참고한 내 기록과 의료 근거 문서를 구분해 확인한다. 기록은 날짜·항목·답변 시점 버전과 사용한 내용을 보여주며, 수정·삭제로 해당 버전을 확인할 수 없으면 발췌를 보류한다. 의료 문서는 6.8의 정확한 위치와 근거문장을 표시하며, 승인된 의료 KB가 없는 개발 단계에서는 미연결 상태를 명시한다.
...
- 앱 잠금은 신규 사용자에게 기본적으로 끈 상태로 제공하며, 설정에서 6자리 잠금 번호를 등록해 선택적으로 켤 수 있다. 기존 사용자가 설정한 잠금은 유지하고, 잠금을 끌 때는 현재 잠금 번호를 확인한다. 잠금을 끄면 기기 인증도 해제한다. 잠금 사용 여부와 무관하게 기록의 암호화 저장과 운영체제 수준의 안전한 키 저장소를 유지하며, 앱 데이터가 암호화되지 않은 자동 클라우드 백업에 포함되지 않게 한다.
- 잠금이 꺼져 있으면 재실행 시 인증 없이 수첩을 열고, 앱을 잠시 벗어났다가 돌아와도 작성 화면을 유지한다. 앱을 벗어날 때 초안을 저장하고 임시 대화와 진행 중인 AI 작업을 정리하는 동작은 잠금 사용 여부와 무관하게 유지한다. 사용자가 선택한 기간 동안 보관하는 대화는 해당 보관 정책을 따른다.
...
- 한국어, 영어, 일본어, 중국어 간체, 중국어 번체 번역 자산을 유지한다. 2026-09-11 사용자 결정으로 문서 및 언어별 지원 준비가 끝날 때까지 언어 변경을 일시 중단한다.
- 잠금 화면 및 설정에 언어 항목을 남기되 비활성화하고 지원 준비 중임을 표시한다. 기존에 선택한 언어는 초기화하지 않고 재실행 후에도 유지한다. 신규 기본값은 한국어다.
- UI, 항목명, 오류, 날짜 선택기, 인증 안내, 향후 예약 알림에 적용한다. 사용자 작성 기록과 기존 진료 준비 원문은 번역하거나 변경하지 않는다.
- 언어 선택은 국가나 의료 기준의 변경이 아니다. 대한민국 긴급 연락 번호임을 표시하며 기존 의료 안전 정책을 모든 언어에 동일하게 적용한다.
