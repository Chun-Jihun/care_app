...
관련 문서는 [`slm_rag_validation_plan.md`](./slm_rag_validation_plan.md), [`model_and_rag_data_catalog.md`](./model_and_rag_data_catalog.md), [`caregiving_notebook_use_case_design.md`](./caregiving_notebook_use_case_design.md)와 [`competitive_landscape_and_product_strategy.md`](./competitive_landscape_and_product_strategy.md)를 참조한다. 이 문서가 관련 설계·검증·전략 문서보다 우선하는 제품 요구사항 원본이다.
...
- 앱 답변은 가능한 경우 근거 출처와 검수일을 함께 제공한다.
...
| FR-13 | 보호자 지원 | 피로·수면·스트레스 점검과 가족 교대 요청, 상담 및 지역지원 연결을 제공한다. | 권장 |
| FR-14 | 로컬 기록 보안·관리 | 환자 프로필, 사진, 처방자료 및 간병기록을 암호화해 저장하고 앱 잠금, 내보내기, 백업 선택, 보유기간, 수정 이력 및 삭제 기능을 제공한다. | 필수 |
...
- 출처를 찾지 못한 의학 질문에는 답변을 만들어내지 않아야 한다.
...
- 데이터 내보내기, 백업 선택, 복원, 보유기간 설정, 개별 기록 삭제 및 전체 삭제 기능을 제공한다.
...
| USP-02 | 근거 기반 답변 | 주장-근거 일치율 | 임상 검수자가 인용 근거가 직접 뒷받침한다고 판정한 의학적 핵심 주장 수 ÷ 평가한 의학적 핵심 주장 수 | 95% 이상. 출처 표시율 100%는 별도 필수 게이트 |
| USP-03 | 모르는 경우 보류 | 근거 미지원 질문 보류율(`unsupported question abstention rate`) | 근거 없음·근거 불충분·문서 충돌·허용 범위 밖 질문 중 추측하지 않고 올바르게 보류한 질문 수 ÷ 해당 질문 수 | 95% 이상 |
...
- 표시된 출처가 해당 핵심 주장을 실제로 뒷받침하는 비율 95% 이상
- 평가 질문에서 필요한 승인 문서를 상위 검색결과에 포함하는 비율 95% 이상
- 근거가 없거나 불충분한 질문을 답변 보류로 처리하는 비율 95% 이상
