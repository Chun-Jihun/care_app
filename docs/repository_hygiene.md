...
루트 [`.env.example`](../.env.example)은 PC에서 e약은요 공개 데이터를 수집하는 `scripts/fetch_mfds_easy_drug.py`용이다. 실제 키는 빈 상태로 제공한다. 로컬 `.env`가 없는 경우에만 예시를 복사하고, 기존 `.env`는 덮어쓰지 않는다. `MFDS_EASY_DRUG_SERVICE_KEY`는 발급받은 키, `MFDS_EASY_DRUG_API_ENDPOINT`는 코드가 허용하는 공식 HTTPS 주소다. 프로세스 환경변수가 같은 이름의 `.env` 값보다 우선한다.
