...
2026-09-07 구현 결정: Android와 iOS를 함께 지원하려는 사용자 선택에 따라 `mobile/`에 Flutter 기반을 구현했다. 이 문서는 전체 목표 스키마를 설명하며, 직접 기록하는 첫 버전의 실제 테이블·암호화·검증 범위는 [`mobile/README.md`](../mobile/README.md)를 따른다. AI·OCR·의학 지식베이스용 테이블은 아직 활성화하지 않았다.

## 목차

2026-09-07 추가 구현: Flutter 저장소 v3는 수동 입력 폼의 `record_draft`와 중복 복원 추적용 `imported_backup`을 사용한다. 이는 아래 OCR·AI 목표 스키마와 구별된다. 초안 보유기간은 사용자가 선택하며 확정 기록과 분리한다. 선택 백업 format 2는 허용한 행만 암호화하고 별도 수첩으로 추가 복원한다. 필터 범위·참조 보존·실패 처리는 [초안·선택 백업 시험 계획 및 결과](./draft_and_selective_backup_safety_plan.md)를 따른다.
