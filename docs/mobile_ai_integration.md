...
1. `mobile/build/app/outputs/flutter-apk/app-release.apk`를 Android arm64 기기에 설치한다.
   이 절의 2026-09-11 산출물은 기존 앱과 동일한 개발 서명이었다. **2026-09-23 이후 새 release 빌드는 정식 키 설정이 필수**이며 디버그 키로 대체하지 않는다. 정식 키 전환 시 기존 개발 앱에 덮어쓰기 설치가 가능한 것으로 가정하지 말고 암호화 백업과 복원을 검증한다. 개발 설치는 debug 빌드를 사용한다.
