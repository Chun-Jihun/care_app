...
11. [검증해야 할 차별화 주장](#11-검증해야-할-차별화-주장)
12. [공식 출처](#12-공식-출처)
...
제품이 완성되기 전에는 아래 표현을 확정적인 마케팅 문구로 사용하지 않는다.

| 후보 주장 | 출시 전에 필요한 검증 |
|---|---|
| 기록이 외부로 전송되지 않는다 | 네트워크·SDK·로그·백업 경로 검사 |
| 모든 의료 답변에 정확한 근거가 있다 | 핵심 주장별 출처 표시율과 실제 뒷받침 비율 평가 |
| 근거가 없으면 답하지 않는다 | 근거 없음·충돌·범위 밖 평가 세트의 보류율 측정 |
| 위험 상황을 놓치지 않는다 | 사전 정의한 고위험 표현·오타·일상어 변형 시험 |
| 처방자료를 정확히 읽는다 | 국내 실제 형식, 저화질·반사·가림 이미지와 사용자 수정률 평가 |
| 간병 부담을 줄인다 | 기록 시간, 진료 준비 시간, 정보 탐색 시간과 사용자 조사 |
| 경쟁 앱과 다르다 | 동일 시점의 공식 기능 재확인과 실제 사용자 대안 인터뷰 |

`유일한 앱`, `완전히 안전한 AI`, `의료 오류가 없는 앱`과 같은 절대 표현은 사용하지 않는다. 차별화는 기능 목록이 아니라 측정된 안전성, 근거 충실도, 개인정보 경계와 실제 간병 부담 감소로 입증한다.
