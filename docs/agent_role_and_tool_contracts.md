...
`v0.1.0`의 결정적 host·trace 참조 구현과 48개 합성 oracle fixture smoke 범위는 [`DS-AGENT 결정적 도구 호스트·trace 파일럿`](./ds_agent_deterministic_pilot.md)에 기록한다. 참조 실행은 계약 기반을 검증할 뿐 실제 모델 성능이나 의료 출시 결과가 아니다.
